...
export class AsaasService {
    ...
    static async createProfessionalAccount(userId: string, config?: { mobilePhone?: string, incomeValue?: number }): Promise<string> {
        try {
            ...
            // Salva a API Key da sub-conta para usar em transferências futuras
            if (newAccount.apiKey) {
                user.asaasApiKey = newAccount.apiKey;
            }
            await user.save();
    ...
    // Realizar Transferência (Saque) - usa a API key da sub-conta do profissional
    static async transferFunds(
        ...
        if (!subAccountApiKey) {
            throw new Error('API Key da sub-conta do profissional não encontrada. O profissional precisa ter uma conta Asaas ativa.');
        }

        // Cria instância com a API key da sub-conta do profissional
        const subAccountApi = axios.create({
            baseURL: process.env.ASAAS_API_URL || 'https://sandbox.asaas.com/api/v3',
            headers: {
        ...
        try {
            ...
            if (operationType === 'PIX') {
                ...
                payload.pixAddressKeyType = AsaasService.detectPixKeyType(pixKey);
            } else {
                if (!bankAccount) throw new Error('Dados bancários obrigatórios para TED');
                payload.bankAccount = bankAccount;
            }

            const { data } = await subAccountApi.post('/transfers', payload);
            return data;

        } catch (error: any) {
            console.error('Erro ao realizar transferência Asaas:', error.response?.data || error.message);
            throw new Error(`Falha na transferência: ${JSON.stringify(error.response?.data?.errors || error.message)}`);
        }
    }

    // Consultar saldo real da sub-conta do profissional no Asaas
    static async getSubAccountBalance(subAccountApiKey: string): Promise<number> {
        if (!subAccountApiKey) return 0;
        ...
        try {
            const response = await subAccountApi.get('/finance/balance');
